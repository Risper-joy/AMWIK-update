...
import { Search, Calendar, MapPin, Users, Clock, DollarSign, Loader2, ArrowRight } from "lucide-react"
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        // Filter for published events with valid dates
        const publishedEvents = eventsList.filter(
          (event: any) => 
            event.status === "Published" && 
            event.startDate &&
            new Date(event.startDate) > new Date(Date.now() - 24 * 60 * 60 * 1000) // Show events from yesterday onwards
        )

        // Sort by start date
        const sorted = publishedEvents.sort((a: any, b: any) => {
